...
class Day05PrintQueue : Day {
    ...
    override fun partB(input: String): Number {
        ...
        val previouslyInvalidUpdates = invalid.mapIndexedNotNull { index, (rule, _) -> if (rule != null) index else null }
        ...
        val nowFixed = reordered.filterIndexed { index, update -> index in previouslyInvalidUpdates }.sumOf { it[it.size / 2] }
        return nowFixed
    }
    ...
    fun List<List<Int>>.findInvalidUpdates(rules: Set<String>): List<Pair<String?, List<Int>>> = this.mapNotNull { update ->
        for (first in 0..<update.lastIndex) {
            for (second in first + 1..update.lastIndex) {
                val check = "${update[second]}|${update[first]}"
                if (rules.contains(check)) {
